...
As above for the ``faidx`` command, there is an output for this command, which is a ``.bai`` However, this file is never specified in the actual command, but needs to be specified in the output for your rule. And again, as above, this file needs to specificed as input for your ``bcftools`` command.
